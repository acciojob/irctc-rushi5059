...
public class TrainService {
    ...
    public Integer calculateAvailableSeats(SeatAvailabilityEntryDto seatAvailabilityEntryDto){
        ...
        //And there are 2 seats avaialble in total in the train and 2 tickets are booked from A to C and B to D.
        ...
        List<Ticket>ticketList=train.getBookedTickets();        // total booked ticket
        String []trainRoot=train.getRoute().split(","); // trains route seperated by comma
        int booked=0;
        for(Ticket ticket:ticketList){
            booked+=ticket.getPassengersList().size(); //  numbers total tickets booked
        }

        HashMap<String,Integer> map=new HashMap<>();
        for(int i=0;i<trainRoot.length;i++){
            map.put(trainRoot[i],i);        // key - train name,  value - no. sequence of upcoming stations
        }
        if(!map.containsKey(seatAvailabilityEntryDto.getFromStation().toString())||!map.containsKey(seatAvailabilityEntryDto.getToStation().toString())){
            return 0;       // if either of the station doesn't exsits
        }

        int seatCount=train.getNoOfSeats()-booked;
